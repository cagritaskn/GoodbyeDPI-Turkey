...
Tarayıcıda Discord açılıp masaüstü uygulaması takılıyorsa (update failed / sonsuz yükleme), önce mevcut GoodbyeDPI penceresini veya hizmetini kapatın, ardından ZIP içindeki ``turkey_discord.cmd`` dosyasını **yönetici olarak** çalıştırmayı deneyin. Bu profil modeset ``-6`` (``--wrong-seq``), Discord host listesi (``lists/discord.txt``) ve Yandex DNS redirect kullanır. Kalıcı kurulum için: ``service_install_dnsredir_turkey_discord.cmd``. Bağlam: [ValdikSS/GoodbyeDPI#779](https://github.com/ValdikSS/GoodbyeDPI/issues/779).
